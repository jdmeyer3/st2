...
class ActionDB(stormbase.StormFoundationDB, stormbase.TagsMixin,
    ...
    enabled = me.BooleanField(
        required=True,
        default=True,
        verbose_name='A flag indicating whether the action is enabled.')
    entry_point = me.CharField(
        required=True,
        verbose_name='The entry point to the action.')
    pack = me.CharField(
        required=False,
        verbose_name='Name of the content pack.')
    runner_type = me.DictField(
        required=True, default={},
        verbose_name='The action runner to use for executing the action.')
    ...
    output_schema = stormbase.EscapedDynamicField(
        verbose_name='The schema for output of the action.')
    notify = me.EmbeddedDocumentField(NotificationSchema)

    class Meta:
        collection_name = "action_d_b"
        indexes = [
                      pymongo.IndexModel([('name', pymongo.TEXT)]),
                      pymongo.IndexModel([('pack', pymongo.TEXT)]),
                      pymongo.IndexModel([('ref', pymongo.TEXT)]),
